...
export default function OverseasRemittanceUpdate({ updateKey, layoutRef, getCopyPage }: any) {
    ...
    async function getDataInfo() {
        await getData.post('remittance/getRemittanceDetail', {
            ...
            // 부분송금액 (송금내역 총액) 계산
            const partialRemittance = remittanceDetail.reduce((sum, row) => sum + ((Number(row.supplyAmount) || 0) + (Number(row.fee) || 0)), 0);
            ...
            setInfo({
                ...
                partialRemittance: formatAmount(partialRemittance, 'USD')
            })
    ...
    function modalSelected(list = [], remittance = '') {
        ...
        setSelectOrderList(prevList => {
            ...
            let partialRemittance = Number(String(remittance || info.partialRemittance || '0').replace(/,/g, ''));
    ...
    function copyPage() {
        ...
        copyInfo['remittanceDetail'] = [...list, ...commonFunc.repeatObject(ORInfo['write']['defaultData'], 100 - list?.length)];
        copyInfo['orderDetailList'] = selectOrderList;

        getCopyPage('overseas_remittance_write', {...copyInfo, _meta: {updateKey: Date.now()}})
    }
